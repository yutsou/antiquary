        ...
        switch ($notice->type) {
            ...
            case 2:
                ...
                return match ($notice->code) {
                    ...
                    1 => ['競標成功', '物品 No.'.$model->id.' ，以 NT$'.number_format($model->current_bid).'賣出'],
                    2 => ['已得標', '物品 '.$model->name.' ，以 NT$'.number_format($model->current_bid).'得標'],
                    3 => ['流標', '物品 No.'.$model->id.' ，無人競標流標，請至平台選擇處理方式。'],
                    4 => ['流標', '物品 No.'.$model->id.' ，未達底價流標，請至平台選擇處理方式。'],
                    5 => ['棄標', '物品 No.'.$model->id.' ，遭到棄標，請至平台選擇處理方式。'],
                };
            case 3:
                $model = Order::find($notice->target_id);
                return match ($notice->code) {
                    0 => ['已收到匯款', '訂單 No.'.$model->id.' ，已收到匯款'],
                    1 => ['訂單已完成', '訂單 No.'.$model->id.' ，已完成'],
                    2 => ['已完成委賣', '訂單 No.'.$model->id.' ，已匯款'],
                };
            ...
            case 5:
                ...
                return match ($notice->code) {
                    0 => ['流標的物品已寄出', '物品編號 No.'.$lot->id.' ，已寄出。'.$logisticInfo->company_name.': '.$logisticInfo->tracking_code],
                };
            case 6:
                $model = Order::find($notice->target_id);
                return match ($notice->code) {
